...
function PlayAudioFile(jrpid, element, starttime) {
	console.log('PlayAudioFile000====',TIMEMAP);
	getTimemap(ID);
   ...
   if (jrpid != AUDIOjrpid) {
      ...
      AUDIOid = element.id;
		...
		if (window.location.href.match(/tasso/i)) {
			source += '<source src="/scripts/test.mp3" ';
		} else {
			//source += '<source src="/scripts/hai_1.mp3" ';
			source += '<source src="/scripts/test.mp3" ';
		}
...
function InitializeTimemap() {
	...
	console.log('InitializeTimemap-TIMEMAP====',TIMEMAP[ID]);
	var increment = 20;
...
function CheckTimeMap(timemap, events, currenttime, increment) {
	...
	console.log("99999900000",target,target.tstamp,currenttime,increment);
...
function LoadTassoData(fn) {
	...
	if (sessionStorage["TASSODATA"]) {
		...
		var request = new XMLHttpRequest();
		request.open("GET", "/scripts/tasso-data.aton");
		request.send();
		request.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				...
				sessionStorage.setItem['TASSODATA'] = JSON.stringify(TASSODATA);
				//console.log("GOT TASSODATA2 ret:::",sessionStorage );
				fn();
			}
		}
	}
...
function GetCatalogInfo(id) {
	...
	console.log('outttt999999999',output);
	return output;
}
